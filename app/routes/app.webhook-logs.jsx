...
import { getWebhooksLogs } from "../models/logs.modal";
...
const emptyState = {
    ...
    text: "Webhook logs will appear here once webhook events are received.",
};
...
export const loader = async ({ request }) => {
    ...
    const paginationData = await getWebhooksLogs(shop, admin.graphql, page, query);
...
export async function action({ request }) {
    ...
    return redirect("/app/webhook-logs");
}
...
const LogsTable = () => {
    ...
    return (
        <div className="table-container">
            ...
            <div className="webhook-logs-table">
                ...
                <s-table paginate={totalCount > 10} hasNextPage={hasNextPage} hasPreviousPage={hasPreviousPage} onNextPage={() => handlePaginate(currentPage + 1)} onPreviousPage={() => handlePaginate(currentPage - 1)}>
                    {selectedIds.length === 0 ? (
                        <s-table-header-row>
                            ...
                            <s-table-header>Tags Applied</s-table-header>
        ...
        <s-table-row id={log.id} clickDelegate={`log-${log.id}`}>
            ...
            <s-table-cell>
                {log.productDeleted ? (
                    ...
                    <s-clickable
                        href={`shopify:admin/products/${log.productId.split("/").pop()}`}
                        accessibilityLabel={`Go to the product page for ${log.productTitle}`}
                        borderRadius="small"
                    >
                        <s-stack direction="inline" gap="small-100" alignItems="center">
                            {log.productImage ? (
                                ...
                                <s-thumbnail alt="No image available" size="small" />
                            )}
                            <s-text>
                                {truncate(log.productTitle)}
                            </s-text>
                        </s-stack>
                    </s-clickable>
                )}
            ...
            <s-table-cell>
                {log.triggerType === "CREATE" ? (
                    <s-badge icon="plus" tone="info">
                        Product Created
                    </s-badge>
                ) : (
                    <s-badge icon="edit" tone="neutral">
                        Product Updated
                    </s-badge>
                )}
            </s-table-cell>
            <s-table-cell>
                {log.appliedTags ? (
                    <div className="d-flex gap-1 flex-wrap" style={{ maxWidth: "150px" }}>
                        {JSON.parse(log.appliedTags).map((tag) => (
...
export default function ActivityLogs() {
    ...
    return (
        <LogsContext.Provider value={{ selectedIds, handleRowCheckboxChange, handleBulkDelete }}>
            ...
            <ClearLogsModal message={"Are you sure you want to clear all webhooks activity logs?"} />
